  ...
  return (
    <>
      <div className="flex mx-auto max-h-[500px] px-10 mt-8 mb-4">
        <img
          className="h-300 mx-100 rounded-xl border border-b-4 shadow-md opacity-50"
          src={`${event.image}`}
        />
      </div>
      <div className="flex flex-col items-center gap-4">
        ...
        <Button
          className="md:w-60 w-40 rounded-full mb-2 text-md md:text-lg font-semibold"
          variant={"outline"}
        >
          Register
        </Button>
      ...
      <div className="mx-10  flex flex-col gap-2">
        <h1 className="md:text-4xl text-xl font-semibold">{event.event}</h1>
        <p className={"md:text-lg text-md" + inria.className}>
          {event.long_desc}
        </p>
      </div>
      <div className="flex flex-col mx-10 mt-8">
        <h1 className="md:text-4xl text-xl font-semibold">FAQs</h1>
        {event.event === "Hackathon"
          ? faqs.hackathon.map((faq, index) => (
              <Accordion
                key={faq.id}
                type="single"
                collapsible
          ...
          : event.event === "Coding"
          ? faqs.coding_contest.map((faq, index) => (
              <Accordion
                key={faq.id}
                type="single"
                collapsible
      ...
      <div className="flex flex-col gap-5 mx-10 mt-8">
        <h1 className="md:text-4xl text-xl font-semibold">Contacts</h1>
